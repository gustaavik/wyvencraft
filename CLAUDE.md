...
| Which tool a block needs | `[block.harvest]` on the block in `assets/blocks.toml`: `tool = "pickaxe"` (or a list, `["shears", "sword"]`) is what mines it at full speed, matched against `[item.tool] kind`; `required = true` additionally withholds the drop from anything else. Omit the table entirely and nothing is good at it (glass, bedrock). **The dependency runs block → tool, never the reverse**, so adding a block is one entry in one file and never an edit to every tool that should be good at it. Speed math is `inventory::mining::break_seconds`, the drop gate is `InGameState::can_harvest`, and `content::unknown_harvest_tools` warns about a block wanting a kind no item declares (`world` sits below `inventory` and cannot check it at parse) |
...
| Tool tiers / melee damage | Tiers are data only — `dig_speed` + `durability` (+ `damage` on swords and axes) in `assets/items.toml`; `kind` is the tier-independent part. There is deliberately **no** harvest-level gate: naming a tool in `[block.harvest]` makes a block *faster* to mine, never gated, and only a block that also says `required = true` withholds its drop (oak leaves, and nothing else). A tool without `damage` swings for `mobs::PLAYER_ATTACK_DAMAGE` (the fist); the local swing resolves in `InGameState::melee_damage`, a client's in `client_melee_damage`, which reads the inventory that client last reported |
