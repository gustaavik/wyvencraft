...
- **Item capabilities** — `inventory::component::ItemComponent`, one impl per
  ...
  `shearable`), stored **in key order** because `Item`'s `Debug` feeds
...
| Add an item / tool / food / armor | `assets/items.toml`: `id` (lowercase, `_`-separated — what `/give` takes and what saves store), optional `display_name`, then whichever **capability** tables apply — `[item.tool]` (`dig_speed`/`durability`/`harvests`, optional `damage`), `[item.consumable]` (`hunger`/`saturation`), `[item.equippable]` (`slot`/`defense`/`durability`), `[item.placeable]` (`block`), `[item.shearable]` (a marker) — plus `[item.model]` with `path`/`scale`/`offset`/`rotation`; starter kit in the same file. An item may carry any combination, and `max_stack` is derived (1 if any capability wears out). A table nothing recognises **rejects the whole file**, naming the key |
...
| Tool tiers / melee damage | Tiers are data only — `dig_speed` + `durability` (+ `damage` on swords and axes) in `assets/items.toml`. There is deliberately **no** harvest-level gate: `harvests` decides *what* a tool is for, never *whether* a block drops — and a tool has no `kind`, because the one thing that used to read it (the leaf drop gate) now asks for a capability instead. A tool without `damage` swings for `mobs::PLAYER_ATTACK_DAMAGE` (the fist); the local swing resolves in `InGameState::melee_damage`, a client's in `client_melee_damage`, which reads the inventory that client last reported |
...
| Block drop rules        | `drops = ...` on the block in `assets/blocks.toml` (`"self"`, `"none"`, `{ requires = "<item capability>" }`, `{ item, count }`). `requires` names an `[item.<capability>]` table — oak leaves ask for `shearable` — so the gate reads what the held item can *do* rather than comparing two free-form strings. `world` sits below `inventory` and cannot see the capability set, so `content::unsatisfiable_drops` checks the names once both registries exist and warns about a rule nothing can satisfy |
...
2. `cargo test --workspace` green (871 tests: 9 app, 2 assets, 54 auth, 11 core,
   116 model, 55 render, 21 voxel, 603 game). One of the game tests
