...
**Wyvencraft** — a voxel game in Rust that looks like Minecraft, plays a
Valheim-style progression loop (concentric biome rings, each with a boss whose
altar is found by reading shrines, each boss unlocking the next biome's ore),
and borrows Terraria's layered underground and tiered gear. Built with **Vulkan via
...
resolved by `src/infrastructure/paths.rs`, so a launcher can replace the install directory on
...
- `WYVEN_PERSPECTIVE=third|front|first` → open in that camera instead of first
  person. F5 needs a human at the keyboard, and everything drawn on the player's
  own body — the model, what it holds — is invisible from inside its head, so
  paired with `WYVEN_SCREENSHOT_AT` this is what makes *those* changes checkable
  too. Applied in `InGameState::build`, so a joining client gets it as well —
  which is what lets a two-process run check both ends of a peer's fist.
- `WYVEN_EDITOR=1` → enable the in-game item placement editor (F6). Off by
  default because it *writes into* `assets/`, which belongs to the install.
  `WYVEN_EDITOR=open` also opens the panel at boot — F6 needs a human at the
  keyboard exactly as F5 does, so paired with `WYVEN_SCREENSHOT_AT` this is what
  makes the panel itself checkable from an automated run.
- `WYVEN_INVENTORY=open` → open the inventory, and the crafting pane above it,
  at boot. E needs a human at the keyboard just as F5 and F6 do, so paired with
  `WYVEN_SCREENSHOT_AT` this is what makes the panel checkable from an automated
  run.
- `WYVEN_DEBUG_SPAWN=cow,zombie,...` → spawn the named mobs next to the player at
  ...
  it never appears on its own). Mobs appear beside the *player*, so this
  composes with the next one.
- `WYVEN_DEBUG_GOTO=meadows_altar` → start 11 blocks south of the nearest
  structure of that id (`assets/structures.toml`), facing it. Walking to a
  shrine needs a human at the keyboard, so paired with `WYVEN_SCREENSHOT_AT`
  (and `WYVEN_DEBUG_SPAWN="elder stag"` for the boss bar) this is what makes a
  structure, the arena and a boss checkable from an automated run.
...
> protocol and the authority model — lives in [`docs/`](docs/README.md). **The layers inside
> the game crate, the ECS and the simulation** are [`docs/architecture.md`](docs/architecture.md). This section is
...
wyven-model    ← core, assets, render   .gltf/.bbmodel/block JSON → ModelMesh + texture, bones + clips
...
| --- | --- | --- |
| `TileSource` (+ `ReservedTiles`) | `wyven-render` | `presentation::art::WyvencraftArt` |
| `BlockCatalog` | `wyven-voxel` | `presentation::content::BlockAppearance` |
| `BlockProperties` | `wyven-voxel` | `domain::world::BlockRegistry` |
| `WorldGenerator` | `wyven-voxel` | `domain::world::NoiseGenerator` |
| `Protocol` / `JoinVerifier` | `wyven-net` | `infrastructure::net::WyvenProtocol` / `infrastructure::net::TicketJoin` |
| `Game` (+ `Screen`) | `wyven-app` | `presentation::screens::Wyvencraft` (+ every screen) |
...
> **On `GameContent`**, hashed registries are `content.rules.{blocks,items,…}` and
> `content.rules.hash()`; everything visual is `content.visuals.{models,tiles,item_icons,…}`.
...
- **Data-driven content** — game content lives in TOML under `assets/`
  ...
  `recipes.toml`), loaded once at startup into `presentation::content::GameContent` and
  shared via `Arc`. Behavior is *components + code hooks*: data declares typed
  components (`drops`, `fluid`, entity params, and every `[item.<capability>]`);
  each component is implemented once in Rust and dispatched on — never on
  block/item identity.
...
- **Ports & adapters** (at I/O boundaries only — never in per-frame/per-voxel hot
  paths): `presentation::content::ContentSource`, `infrastructure::save::WorldRepository`, `application::session::Session`,
  ...
  `presentation::editor::{PlacementStore,Stamps}` are the newest pair, and the reason the file
  ...
  `Wyvencraft::start` via `infrastructure::audio::open_default_backend`.
  `domain::chat::CommandContext` is a port for a different reason — not I/O, but to invert
  ...
  `SessionContext` in `presentation::screens::ingame_state::chat`, double `domain::chat::FakeContext`.
- **Chat commands** — `domain::chat::command::ChatCommand`, one impl per file, found
...
| Block hitbox (crosshair, outline, cracks) | Not authored — `presentation::content::placed_bounds` measures the placed model and `wyven_voxel::model_hitbox` turns it into a square, centred, cell-clamped box on `BlockModel::hitbox`, so it can never drift from what is drawn. The raycast predicate returns `wyven_voxel::Target::{Cell,Box}`; a `Box` the ray misses does **not** stop the march. `InGameState::{target_at,hitbox_at}` are the single source for targeting *and* both overlays. Mob line-of-sight deliberately stays `is_solid` + `Target::Cell` — a flower must not hide you |
| Add an item / tool / food / armor | `assets/items.toml`: `id` (lowercase, `_`-separated — what `/give` takes and what saves store), optional `display_name`, then whichever **capability** tables apply — `[item.tool]` (`kind`/`dig_speed`/`durability`, optional `damage`), `[item.consumable]` (`hunger`/`saturation`), `[item.equippable]` (`slot`/`defense`/`durability`), `[item.placeable]` (`block`) — plus `[item.model]` with `path`/`scale`/`offset`/`rotation`; starter kit in the same file. An item may carry any combination, and `max_stack` is derived (1 if any capability wears out). A table nothing recognises **rejects the whole file**, naming the key. **A tool never names a block:** `kind` says only what shape it is, and the *block* says which shapes it wants — see the next row |
| Which tool a block needs | `[block.harvest]` on the block in `assets/blocks.toml`: `tool = "pickaxe"` (or a list, `["shears", "sword"]`) is what mines it at full speed, matched against `[item.tool] kind`; `required = true` additionally withholds the drop from anything else. `tier = N` additionally makes it unbreakable by any accepted tool below tier N (see *Tool tiers*). Omit the table entirely and nothing is good at it (glass, bedrock). **The dependency runs block → tool, never the reverse**, so adding a block is one entry in one file and never an edit to every tool that should be good at it. Speed math is `domain::inventory::mining::break_seconds`, the drop gate is `InGameState::can_harvest`, and `presentation::content::unknown_harvest_tools` warns about a block wanting a kind no item declares (`world` sits below `inventory` and cannot check it at parse) |
| Add a *new kind* of item behaviour | **one new file in `src/domain/inventory/component/` + one entry in `COMPONENTS`** (the `ChatCommand`/`ModelLoader` pattern). Implement `ItemComponent` (`key`/`clone_box`, plus `stack_limit`/`durability` if it wears) and its `ComponentParser`. Nothing in `Item`, the loader, or the stack rules changes. If the capability does something on right-click, that half is code: one `try_*` function in `presentation::screens::ingame_state::interaction` and one line in its `USE_HOOKS`, which is where the precedence between usable capabilities is stated. A `parse` returning `Err` fails the file (a structural fault); `Ok(None)` degrades that one capability with a warning (an unresolved reference) |
| Tool tiers / melee damage | Speed is data — `dig_speed` + `durability` (+ `damage` on swords and axes) in `assets/items.toml`; `kind` is the tier-independent part. **Progression is a gate**: `[item.tool] tier` against `[block.harvest] tier`, checked by `domain::inventory::mining::meets_tier` — a block whose tier the held tool of an accepted kind does not reach cannot be broken *at all* (`break_seconds` is `INFINITY`, and `InGameState::hint_tier` says once why). Only biome ores carry a tier; everything else stays mineable by hand, and only a block that says `required = true` withholds its drop (oak leaves). The host re-checks the tier on a client's `Break` against its last-reported held tool and undoes a refused one. `domain::content::references::unreachable_tiers` warns about a tier no tool meets (cinder ore, until the Ashlands gear lands). A tool without `damage` swings for `application::simulation::PLAYER_ATTACK_DAMAGE` (the fist); the local swing resolves in `Simulation::melee_damage`, a client's in `client_melee_damage`, which reads the inventory that client last reported |
| Load a 3D model from a file | drop a `.gltf`, `.bbmodel` or `.json` (Blockbench *Java Block/Item*) in `assets/models/items/` (or `assets/models/blocks/` for block geometry), then point at it: `[entity.visual] kind = "model"` in `assets/entities.toml`, or `[item.model]` in `assets/items.toml`. For a *character* — bones and clips, drawn against an atlas skin sheet — use `kind = "rigged"` instead (its own row below). Parsing is `wyven_model::{gltf,bbmodel,javamodel}` behind the `ModelLoader` trait (a new format = a new impl + one line in `ModelRegistry::LOADERS`); placement math in `wyven_model::mesh`; GPU textures uploaded lazily in `presentation::screens::ingame_state::view`. Exports disagree on which plane a flat object lies in (the tiered tools are flat in XY, `vine_sword` in YZ), so `ModelSpec::rotation` turns a model about its own axes — applied after `offset` re-centres it. **A `.json` can place itself instead — see the next row** |
| A rigged, animated character | Author it in Blockbench with a **named bone hierarchy** and keyframe clips, save the `.bbmodel` under `assets/models/entity/<name>/`, then `[entity.visual] kind = "rigged"` with `path`, `scale` and an optional `skin` (a `presentation::art::mobskin` sheet name; omitted = the player skin block). `scale` fits the authored figure to the entity's own `[entity.physics] height` — the player model is 1.096875 blocks tall and its collision box is 1.8, hence `1.641026`. Parsing is `wyven_model::{rig,clip}`, driven from `presentation::render::rigged::HumanoidRig`, baked by `Character::bake`. Bones and clips are found **by name** (`head`, `arm_l`/`arm_r`, `hand_l`/`hand_r`, `walk`, `run`, optional `idle` and `jump`); anything missing degrades — no clip, no head turn — rather than failing to load. **Handedness is resolved by pivot, not by label**: the shipped player names its arms from the viewer's side, so `hand_l` is the character's *right* hand, and `rigged::right_of` picks whichever candidate sits at `+X`. Clip time comes from `AnimationState::walk_phase`, which advances with *distance travelled*, so stride matches ground speed; `idle → walk → run` crossfades on `walk_amount` and `AnimationState::speed`. **`jump` is not on that clock at all** — it is sampled by *vertical velocity* (launch at `+jump_speed`, apex at 0, fall at `-jump_speed`) and blended over the gait by `AnimationState::air_amount`, so the tuck lands at the real apex whatever the jump's height and stepping off a ledge starts partway in, already falling. Head look and the attack swing are **additive on top** of the clip, because no keyframe can know where the mouse is. The engine never learns the word "walk" — it hands out named bones and clips and the game decides what they mean |
| Airborne / jump animation | a `jump` clip on the character's `.bbmodel` (`loop = "hold"`, keyframed span == declared length). Sampled by **vertical velocity**, not by a clock — `HumanoidRig::airborne` maps `+jump_speed → 0`, `0 → mid`, `-jump_speed → end`, so a jump cut short by a ceiling reverses instead of playing on, and a long fall holds the last pose rather than wrapping. Blended over the gait by `AnimationState::air_amount`, which ramps at `AIR_BLEND_RATE` so landing is not instant in the pose the way it is in the physics. The gait keeps accruing underneath, so a running jump lands back into the stride it left. What counts as airborne is `domain::entity::Motion`: the local player and mobs read their own `on_ground`, while remote players and remote mobs — whose grounded flag never crosses the wire — use `Motion::observed`, which infers it from vertical speed above `OBSERVED_AIR_SPEED` so a stair climb does not read as a hop |
| Add a clip to a character | Keyframe it in Blockbench and name it; nothing in Rust changes unless the *game* has to choose it. `rotation` and `position` channels, `linear`/`catmullrom`/`step` interpolation (anything else warns and falls back to linear). Values are read as **strings** — Blockbench stores molang there — and converted to radians and blocks at parse, so nothing downstream carries a Blockbench unit. A looping locomotion clip is driven by stride, so it plays its *keyframed* span (`Clip::end`) rather than its declared `length`; authoring the two differently logs a warning naming the clip |
...
| Position a **block** in the hand | Same editor, the first row of its dropdown — but **one placement serves every block item**, because every one of them is drawn as the same cube. It lives in `assets/models/items/block.json`, a display-only model the way Minecraft's `block/block` is a display-only parent, loaded by `presentation::content::load_block_item_display` and falling back to `presentation::render::viewmodel::default_block_display` if the file is missing. Only the two hand contexts: a block's inventory icon is painted by `presentation::ui::icon` and a block lying on the floor is sized by its drop entity, so neither passes through a `display` entry and the panel does not offer those tabs. A block that needs its *own* placement is a block that wants a `[block.model]` and an `[item.model]`, which is per-item and already editable |
| Position an item in the hand, the slot or on the ground | **Don't edit the numbers by hand** — run with `WYVEN_EDITOR=1` and press F6. The panel edits whichever layer is actually placing that context (a `display` entry in the model's own `.json`, or `[item.model]` for a `.bbmodel`/`.gltf` — exactly the choice `local_transform` makes) and saves back into that file, leaving every other byte of it alone. It also watches every model file, so a Blockbench re-export or a text-editor change lands in the running game within a frame. Reload is **placement only**: new geometry or a repainted texture still needs a restart, and a `gui` edit is saved for real but shows on the *next* launch, because the icon sheet is rendered once inside `Game::start` — the only moment a `&mut Gui` exists. Code is `src/presentation/editor/` (pure: the value, the two file formats, the watcher, the session) + `src/presentation/ui/editor.rs` (the panel) + `presentation::screens::ingame_state::editor` (the glue). It reaches the renderer through `presentation::editor::PlacementSource` on `view::ModelContent`, which is the only thing the five placement seams know about |
| Where a held/dropped/icon model sits | `display` in the model's own `.json` (`wyven_model::display`), Minecraft's block: `firstperson_righthand`, `thirdperson_righthand`, `gui`, `ground` are wired; `fixed`/`head` parse but nothing draws them. **Each hand context is measured against a frame, and both frames are Minecraft's**, which is what lets an author drag a model into place in Blockbench and have it land here unchanged: first person is `viewmodel::HAND_OFFSET` in camera space, third person is `rigged::hand_space` — the hand bone tipped a quarter turn so `+X` is right, `+Y` *forward* and `+Z` up. Neither is a tuning knob; `rigged::GRIP` is the third-person one, and it places our rig's fist, which no `display` entry says anything about. `wyven_model::mesh::local_transform` is the **one** place that chooses between an authored entry and the `[item.model]` `scale`/`offset`/`rotation`, reached through `presentation::content::ItemModel::local(models, context)`. An entry replaces the spec for that context rather than composing with it — the two centre a model differently (`display` on all three axes, `offset` only in XZ). A `.bbmodel` declares none, so it keeps exactly the placement it always had. `assets/models/items/wooden_sword.json` is the worked example; the numbers are the ones Blockbench's own display preview writes, so authoring is "drag it into place and export" |
| First-person hand       | `presentation::render::viewmodel` (pure: `HandPose` → one `frame()` matrix, swing curve, walk bob, arm mesh). Both the arm and the held item hang off that **one** frame, so an item cannot drift out of the fist. `arm_mesh` bakes only the right arm's bone subtree (`Rig::subtree`) and shifts it so the hand bone's pivot sits at the frame origin, which is where the held item is. It bakes that limb at the rig's **rest** pose and takes no `AnimationState` at all: in first person the camera *is* the head, and a gait authored to read on a whole third-person body throws the elbow across the entire screen when it is the only thing on it. The bob and the swing already live in the frame, so taking the clip as well applied both twice. **The signature is the guarantee** — there is no pose parameter to pass the wrong thing to. Meshes built in `SceneCache::update_hand_meshes` — the branch of `update_player_mesh` that used to `return` — and drawn through `SceneFrame::foreground`, which the renderer records **last, after clearing depth**, so a wall the camera is pressed against cannot slice the hand. Its own fixed `HAND_FOV_DEGREES`, so a wide world FOV does not stretch your own arm. `HAND_OFFSET` is Minecraft's `(0.56, -0.52, -0.72)` and is **not** a tuning knob: it is what makes a Blockbench `firstperson_righthand` entry land where its author dragged it. `ARM_PITCH`/`ARM_YAW`/`ARM_ROLL` are the knobs, because they place *our* arm, which Minecraft says nothing about |
| Head turn / body follow | `domain::entity::animation` (pure). A humanoid is drawn at `AnimationState::body_yaw()`, **not** at its look yaw: standing still the head turns alone inside `FREE_HEAD_TURN`, past it the torso eases after it (`BODY_FOLLOW_RATE`) and is hard-capped at `MAX_HEAD_TURN`, and walking squares the torso up under the look direction (`BODY_WALK_RATE`) — those four are the knobs. `AnimationState::head_offset` is the *offset* from the torso — applied to the rigged player's `head` **bone** in `presentation::render::rigged`, and as a box's `local_yaw` in `model::push_box` for mobs. `Player::yaw` / `Mob::yaw` still drive the camera, the walk basis, mob locomotion and the wire — body yaw is render-only and **derived**, so a remote player's torso is worked out from the look yaw already on the wire and `application::protocol` needs no new field. A mesh and its hand anchor must be built from the *same* pose and yaw or the held item leaves the fist — for the player they are literally the same `rig::Pose`, so they cannot come apart |
| Third-person camera | `domain::entity::camera` (pure: eight rays from a cube around the eye toward the desired spot, nearest hit wins — a single centre ray misses a block the near plane's *corner* still clips). The berth is `wyven_render::Camera::near_radius`, derived from `near`/`fov_y`/`aspect`, so widening the FOV widens the clearance instead of clipping again. `THIRD_PERSON_DISTANCE` in `presentation::screens::ingame_state` is the *desired* distance; `InGameState::world_camera` resolves it against the world, because the view deliberately cannot reach a `World`. It snaps rather than eases: the distance holds no state, which is what lets the world camera and the nameplate camera agree within a frame without sharing any — and `world_camera` is now the **only** place a world camera is built, so they no longer agree merely by being spelled the same way twice. `Perspective::shot` is the one place that knows which way the camera sits, returning a polar `domain::entity::camera::Shot`; polar because the inventory blends between two of them, and a Cartesian lerp from behind the player to in front of them passes through their head at the halfway point. First person is not a special case — it is `ThirdBack` at zero distance |
| Armor (slots, defense, wear) | data in `assets/items.toml` `[item.equippable]`; four slots (helmet/chestplate/leggings/boots) at 36..40 in `domain::inventory::inventory`, sized from `ArmorSlot::ALL` — everything downstream derives from `ARMOR_SIZE`/`TOTAL_SLOTS`, including the wire array, so adding a slot back is a one-line change plus art; defense math in `domain::entity::player::damage`; equip gate + wear in `presentation::screens::ingame_state` and `Simulation::step_player`; net via `ServerMessage::PlayerEquipment`, which carries a whole `Equipment` — armor *and* the main hand — as one value, so the host's "only re-send on change" check cannot go stale on half of it. **Armor is not drawn.** The inflated-shell renderer was removed with the box player model: it addressed the skin sheet through Minecraft's unwrap, and `player.bbmodel` carries its own. Bringing it back means authoring armor as its own `.bbmodel`s over the same rig, not restoring the shells. `presentation::art::armor` still reserves its atlas band (`mobskin` rows are measured from it) but nothing samples it. **Saves are positional against the current `TOTAL_SLOTS`** and armor is the last region, so a save with more armor slots than the build truncates cleanly — which holds only while armor stays last |
| Item icons              | `ItemIcon` is computed in `content`: `Cube` (from block faces), `Flat` (`assets/textures/items/<item id>.png` — the file stem *is* the id), or `Model` for items with `[item.model]`, which now includes every flat item (see *A 2D item*). Drawn by `presentation::ui::icon::draw_item_icon`; the atlas and the 3D icon sheet are registered with egui in `presentation::screens::shared`. A `block_model` block's cube faces are 32px stand-ins **downsampled from its own 256px art** (`presentation::content::derive_face_tiles`), so the icon and the dropped-item cube keep working unchanged — read them through `GameContent::face_textures`, never `Block::textures` |
...
| Live player preview     | There is no offscreen pass — the preview *is* the world. Opening the inventory blends the world camera to `domain::entity::camera::Shot::inspect`, which stands in front of the player's **body** yaw and slides the image left with `Camera::projection_offset` so the model sits in the column the panel leaves clear. Camera in `InGameState::{world_camera,camera_shot}`; the body is forced on past `INSPECT_MODEL_FROM` in `SceneCache::update_player_mesh` (first person has no body mesh otherwise, and culling is off, so it cannot start at 0 or you see the inside of the head) |
| The inventory's open/close animation | `presentation::screens::ingame_state::inventory::OpenAnim` — one linear `t`, eased on read, read by *both* the camera and the panel so they cannot desynchronise. Linear-and-eased-on-read is what makes an interrupted sweep resume rather than restart; smoothstep rather than the exponential blend `domain::entity::animation` uses, because both endpoints must be reached exactly |
...
| Add an ECS component    | a plain type — rules on it live in `domain` (like `ItemDrop`, `Projectile`, `Mob`), bodies and bookkeeping in `application::ecs::components` (`Transform`, `Velocity`, `Body`, `Kind`, `Animation`, `Replica`, `LastSeen`). Nothing registers it: `wyven_ecs` stores any `'static` type. Add it to the relevant bundle in `application::ecs::spawn`. If it must be saved or sent, the DTO mapping goes in `infrastructure::save` / `application::protocol::mapping`, never on the component |
| Add an ECS system       | a plain function in `application::ecs::systems::<concern>` taking `&mut Ecs` plus exactly what it reads (a world predicate, a target list, `dt`), returning what it could not apply itself (hits, steps, reaped mobs). Mutable queries are closures (`ecs.for_each_mut::<(&mut A, &B, Option<&C>, With<D>, Without<E>), _>`); structural changes mid-loop go through a `CommandBuffer`. Call it from `Simulation::tick_entities` (every peer) or from the use case that owns it, in an order you state. Test it against a bare `Ecs` |
| Add a simulation use case | a method on `application::simulation::Simulation` in the file for its concern (`drops`, `mobs`, `player`, `bosses`). It may touch the world, the ECS, the player and the rules, and tell peers through `self.emit(ServerMessage)` (drained by the network pump, broadcast only by a host). Anything that chats, draws or asks the session mid-way stays on `InGameState` as glue. Test it with `Simulation::new(Registries::builtin(), seed, start)` — no screen, socket or GPU |
| Find an entity by id    | `application::ecs::systems::mobs::find(&ecs, MobId)` and `systems::players::{find,get,get_mut,entry}(&ecs, PlayerId)` — linear scans, deliberately: tens of entities, and no second index to keep in step. Wire and save formats still spell ids; `Entity` handles never leave the process |
| Entity tuning / new kind | `assets/entities.toml` (physics/movement/vitals/item/mob components); a new *behavior* = one new component in `domain::entity::kind` + its code hook |
| Add / tune a mob        | `assets/entities.toml` (`[entity.mob]`: health, speeds, `behavior`, `knockback_resistance`, `[entity.mob.ranged]`, `drops`; `[entity.visual]` humanoid `skin=`/`arms_forward` or quadruped) + a `[[spawn]]` entry in `assets/spawning.toml` + a variant in `presentation::art::mobskin::MobSkin`. Art is `assets/textures/entity/<name>/<name>.png`, 64×64 or 64×32 |
| A mob's skin unwrap     | Humanoids use the player unwrap in `presentation::art::skin` and need nothing declared. Quadrupeds carry theirs as data — `head_uv`/`body_uv`/`leg_uv` in `[entity.visual]`, Minecraft's `texOffs` — because a cow, a pig and a sheep all sit at different offsets. `body`/`head`/`leg` px sizes are the box as it **stands**; the body's unwrap is drawn upright and `QuadrupedModel::build_mesh` tips it a quarter turn, which is why its `SkinPart` swaps height and depth |
| Mob AI behavior         | `domain::entity::brain` (pure state machine: Idle/Wander/Chase/Flee, perception → intent); the body is `domain::entity::mob` — `Mob`'s five steps think/steer/integrate/animate/act, which `application::ecs::systems::mobs::simulate` runs as passes; perception, the tick and its attacks are `Simulation::{tick_mobs,resolve_mob_attacks}`, boss beats and kills glue in `presentation::screens::ingame_state::{mobs,bosses}`. Disposition is the `domain::entity::kind::Behavior` enum (`passive`/`hostile`/`inert`) — a new disposition is a variant plus its arm in `MobBrain::think`, never a new boolean |
...
| Mob spawning rules      | `assets/spawning.toml` (caps, ring distances, weights, groups, night rules, `biomes` — strict: an unknown entity or biome rejects the file); planner in `domain::entity::spawning` (pure, seeded; biome via an injected `biome_at`); world sampling in `Simulation::update_spawning` |
| Cross-file references   | `domain::content::references` — altar → boss, offering → item, summon → mob, shrine structure → a readable block, ore tier → some tool. Warned at load (they cross files loaded in an order where the target is not yet visible); the shipped data must report none, which a test pins |
| Placeholder art         | `scripts/gen-placeholder-art.py` (stdlib only) derives every new block, item sprite and mob skin from existing art. Replace any output with real art and drop its recipe from the script |
| Projectiles             | `domain::entity::projectile` (ballistic `Arrow`); launch tuning in `[entity.mob.ranged]`; ticked in `Simulation::fly_arrows` |
| Change terrain          | `assets/worldgen.toml` (⚠ alters existing worlds' unexplored terrain; bump `SAVE_VERSION` and regenerate `worldgen_golden_hashes` deliberately). Noise fields and mesas stay in `domain::world::generation::noise`; the per-column pipeline is `generator` |
| Biomes (Valheim rings)  | `[[biome]]` in `assets/worldgen.toml`, **innermost first**: `id`, `tier`, `start` (inner radius), terrain shape (`base_offset`/`amplitude`/`mesa`), blocks, vegetation, tints. The ring a column is in is `domain::world::generation::biome::pick_ring` over a distance **warped** by noise (`[terrain] ring_warp`) and jittered, so borders wander; shape and tint blend across `ring_blend` so a border is never a cliff. **Every** consumer asks `domain::world::generation::Terrain` (`column`/`biome_at`/`height`/`tint`) — the generator, features, structure placement, spawning and the HUD — so they cannot disagree about where a biome begins. Its id is what `structures.toml`, `spawning.toml` and ore `biomes` name |
| Underground layers / ores | `[[layer]]` in `assets/worldgen.toml` (top to bottom, the last reaching y = 1): `fill`, a marbled `mix` block (a biome's `strata` replaces the first layer's), per-layer cave density, and a `pool` that floods the deepest caves. Ores carry `biomes` and `layers` filters — each biome's metal lies only under that biome. Code in `domain::world::generation::underground` (pure: `underground(noise, config, ground, x, y, z) → Fill`) |
| Structures (shrines, altars) | `assets/structures.toml`: a template (text layers + palette), a `biome`, a `grid`/`chance_per_mille` placement, `reveals`, an optional `guarantee = { within }` near spawn and `arena`. **Pure and seed-deterministic**: `domain::world::structure::Structures` answers `instances_touching(chunk)`, `nearest(structure, from)` and `instance_at(pos)` from `(seed, terrain)` alone, so a host can validate and reveal without generating a chunk; `stamp` writes one instance's cells into one chunk after features. An instance stays inside its own grid cell (the config rejects a grid too small for its reach), which is what makes cross-chunk stamping independent. Strict like worldgen, and part of `content_hash` |
| A block that does something on right-click | `[block.interact] kind = "shrine"` or `kind = "altar", boss = "<entity>"` in `assets/blocks.toml` → `domain::world::block::Interaction`, dispatched by an exhaustive `match` in `presentation::screens::ingame_state::block_use::use_block` (a new kind is a variant plus its arm). Block uses win over held-item `USE_HOOKS` and work empty-handed. The authority confirms the block against the **seed** (`Structures::instance_at`), so a hand-placed wayrune is just stone; a client sends `ClientMessage::UseBlock` carrying its inventory, which the host adopts before judging an offering (a separate sync could be reordered behind it — `Channel::Reliable` is unordered) |
| Bosses                  | `[entity.boss]` on a mob in `assets/entities.toml`: `title`, `arena_radius`, `leash_seconds`, `offering`, `[[entity.boss.attack]]` (`effect` = `melee`/`slam`/`volley`/`summon` — `domain::entity::boss::AttackEffect`, a new kind is a variant plus its arm in `presentation::screens::ingame_state::bosses::land_attack`) and `[[entity.boss.phase]]` (validated at load: a phase naming a missing attack rejects the file). Decisions are the pure, seeded `domain::entity::boss::BossBrain` (wind-up → release → recovery, phases only advance); locomotion is still `MobBrain`'s chase. The fight — summon at an altar, telegraphs, leash, defeat, shared loot — is `presentation::screens::ingame_state::bosses`. Bosses are never saved in `mobs.dat` |
| World progression       | `domain::progression::WorldProgression` (shrines read, structures revealed, bosses beaten) — owned by the authority, saved as `progression.dat`, mirrored to clients whole as `ServerMessage::Progression` on join and on change. Not in `content_hash` (it is world state). Chat commands `/locate <structure>` and `/progress [reveal|defeat|reset]` (op) |
| Compass / boss bar      | `presentation::ui::compass` (pure `strip_offset` + painter) fed by `InGameState::waypoints`, bearings from `domain::progression::waypoint::bearing` (the player's own yaw convention); `presentation::ui::boss_bar` fed by `InGameState::boss_bar`, which reads every boss entity — simulated (its `Boss` component) or replicated (its `Replica` phase), with `Health` either way, the replica's updated by `MobHurt`/`BossPhase` |
| Trees/boulders/features | shapes+chances in `assets/worldgen.toml`; canopy strategies in `domain::world::generation::features` (jittered-grid anchors) |
| Ground cover / scatter  | per-biome `plants = [...]` + `plant_chance_per_mille` in `assets/worldgen.toml`; placement in `domain::world::generation::features::try_plant`, which runs **after** trees and only into air so it can never punch a hole in a trunk |
| Meshing                 | `wyven_voxel::meshing::culled` (face culling; greedy is a TODO)                                  |
| Water / fluids          | `[block.fluid]` component in `assets/blocks.toml` (auto-registers flow blocks); sim in `domain::world::fluid` (fluid-agnostic, ticked from `presentation::screens::ingame_state` with the block registry passed in — the engine's `World` has no registry to ask) |
| Fluid art / animation   | `[block.fluid.texture]` on the same `[[block]]`: an animation strip of `frames` square frames stacked top to bottom, **column 0 flowing, column 1 still** (one column serves both). Loaded by `presentation::content::load_fluid_texture` into consecutive layers via `wyven_render::block_textures::resolve_strip`, meshed in the fluid branch of `wyven_voxel::meshing::culled` — source blocks are still on every face, flowing blocks still on top/bottom and flowing on the sides. `fps` defaults to 8 and the loader **rejects** an `fps * 3600` that is not a whole multiple of `frames` — the shader's animation clock wraps hourly and would otherwise jump mid-swell. `tint` is a `tintindex` (2 = water); `opacity` (0..1) rescales the art's alpha, and is the *only* thing controlling how much of the riverbed shows through, because a body of fluid is one blended sheet however deep it is (its interior faces are culled). A fluid still needs an atlas stand-in for its inventory icon, derived from its first still frame in `content` — read it through `GameContent::face_textures` |
| Player movement/physics | numbers in `assets/entities.toml`; formulas in `domain::entity::player`, `domain::entity::physics`         |
| Crafting recipes        | `assets/recipes.toml`: `output`, `count`, `ingredients`, and an optional `station` (none means by hand, anywhere). Shapeless, **in file order**. The pane lists recipes in that order, grouped by station, so the order never shuffles as the inventory changes. A recipe naming an unknown item or station is skipped with a warning. The host's book is authoritative and travels in `Welcome` (`RecipeData.station` included). Rules are pure, in `domain::inventory::crafting::{craft,station,discovery}`. `craft` is **all or nothing**: it works on a clone of the inventory and commits only if the whole output fits, so a full inventory refuses with "Inventory full" instead of dropping items. Glue is `presentation::screens::ingame_state::crafting`; the pane is `presentation::ui::crafting`, laid out above the inventory panel by `presentation::ui::inventory::layout` (survival only). Crafting is local on every peer, like any other inventory change |
| A crafting station      | `station = "<name>"` on a `[[block]]` in `assets/blocks.toml`, then `station = "<name>"` on the recipes that need it. Reach is `domain::inventory::crafting::STATION_RADIUS` (a sphere around the eye), scanned by `stations_near` while the panel is open and again at the moment of a craft. Right-clicking a station opens the panel (`InGameState::open_targeted_station`, checked in `frame.rs` before `use_selected`); sneaking skips that, so a block can still be placed against it. The pane's sections follow block order (`station_ids`). Art for `workbench` and `forge` comes from `scripts/gen-placeholder-art.py` (`STATIONS`) |
| Recipe discovery        | Valheim-style: a recipe is revealed once the player has held any of its ingredients, or its output. What is remembered is the **items** (`domain::inventory::crafting::KnownItems`), not the recipes, so a changed recipe file never invalidates progress. `InGameState::tick_crafting` learns from the inventory every frame, which catches pickups, `/give`, crafts and restores in one place, then announces new recipes in chat and marks them NEW until selected. The first pass after loading is silent. Saved in `discovery.dat`, fail-soft: a world without it re-learns from what its players carry. A client reports its set with `ClientMessage::SyncKnown`; the host merges it by identity and returns it in `PlayerRestore::known_items` |
| A new screen            | implement `wyven_app::Screen`, push/replace via `Transition`                                |
| HUD / inventory UI      | `presentation::ui::hud`, `presentation::ui::inventory`. Slot art is `presentation::ui::slot` — **one** painter for the hotbar and the grid, because the hotbar *is* the grid's bottom row and the two hand off to each other mid-animation; any difference between them shows as a one-frame pop. Panel chrome is `presentation::ui::ninepatch` against `assets/textures/gui/panel.png` (regenerate with `scripts/gen-gui-textures.py`) |
| Where the inventory panel sits | `presentation::ui::inventory::layout` (pure) and `presentation::ui::hud::hotbar_rect` (pure). Both endpoints of the unfold are **computed**, never read back with `ctx.memory().area_rect()` — an anchored egui `Area` is placed from the size it had last frame, so a read-back is stale on frame one and absent on frame zero, which is exactly the frame E is pressed. `layout` also reports `stage_center_x`, which is where the camera's lens shift comes from, so the panel and the model cannot overlap |
| Item names on screen    | Read them through `GameContent::item_display_name` — never `Item::id`, which is the machine key. Slot tooltips come free from `presentation::ui::inventory::View::slot` (the one path the storage grid, hotbar row and armor column share); the fading label above the hotbar is `presentation::ui::hud::draw_held_label`, timed by the pure `domain::inventory::HeldLabel` that `InGameState` ticks each frame |
| Add a chat command      | **one new file in `src/domain/chat/command/` + one entry in `COMMANDS`** — nothing else changes (the `ModelLoader::LOADERS` pattern). Implement `ChatCommand` (`name`/`usage`/`permission`/`run`); the command parses its own arguments and phrases its own messages. It reaches the world only through the `CommandContext` port, so it never sees a `PlayerId` and works identically for the local player and a remote client. Test it against `domain::chat::FakeContext` with no world, socket or GPU. **Caveat:** a command needing a capability the port doesn't expose yet also grows `CommandContext` + its two impls — and if that capability must reach a *client*, a `ServerMessage` too (`GrantItems`, `Teleport`) |
| Authorize a player      | `ops.toml` in the data directory (`ops = [{ id = "<account uuid>", name = "..." }]`), parsed by `domain::chat::ops`. Keyed by the **account id from the verified join ticket**, never by anything a client asserts. The host/singleplayer player is always an op; only the authority loads the file |
| Chat log / input bar    | `domain::chat::{log,composer}` (pure state), drawn by `presentation::ui::chat::draw_chat`; keys `chat`/`chat_command` in `presentation::config::Keybinds` (T and /) |
| Networking              | `wyven_net::{server,client}` + `application::protocol` transport; role behind `application::session::Session` (Singleplayer/Host/Client + `FakeSession`); message application in `presentation::screens::ingame_state::net` |
| The multiplayer screen  | `presentation::screens::multiplayer_menu` — a list of saved servers, each row showing its name, players online and ping. Three pieces: `browser::ServerBrowser` is the state and rules (selection, two-click delete, which rows may be joined) with no egui and full unit tests; `presentation::ui::multiplayer_menu` draws it and reports an `MpAction`; `MultiplayerMenuState` owns both plus the dialog text. Double-clicking a row joins it. **Hosting is not here** — it is a per-world button in `presentation::screens::singleplayer_menu`, beside Play and Delete |
| Saved servers           | `infrastructure::net::serverlist` — `ServerEntry`/`ServerList` (pure, TOML, fail-soft per entry) behind the `ServerStore` port (`FileServerStore` → `servers.toml` in the data directory, `InMemoryServerStore` for tests). An entry keeps the address **as typed**, so a saved hostname survives its IP changing and no DNS lookup ever runs on the frame that draws the list — `infrastructure::net::address::resolve` is called from a worker |
| Player count and ping   | There is no side-channel: a probe *is* a client. `infrastructure::net::status::NetStatusProbe` presents a real join ticket, sends `ClientMessage::RequestStatus`, times the `ServerMessage::Status` that comes back and disconnects. Behind the `StatusProbe` port (`FakeStatusProbe` is the double). The host answers in `presentation::screens::ingame_state::net::report_status` **without announcing or recording the peer** — see `Peers::announced` and the next row. One ticket per Refresh, never reused across refreshes (a host refuses a nonce it has seen) |
...
| Player nameplates       | `presentation::ui::nameplate` painted from `InGameState::draw_nameplates`; projection is `wyven_render::Camera::project`. egui composites after the world pass with no depth, so occlusion is an explicit `wyven_voxel::raycast` against `is_solid` |
| Saving / world files    | `save` module (formats, `saves/<slug>/`) behind `infrastructure::save::WorldRepository` (File/Null/InMemory); triggers in `presentation::screens::ingame_state::save_world` |
| Pipelines / passes      | `wyven_render::pipeline`, `wyven_render::renderer`                                                     |
| GPU meshes / camera     | `presentation::screens::ingame_state::view` (`SceneCache`) — the only holder of `RenderContext`             |
| Startup / dev env vars  | `application::boot_plan::BootPlan::from_env` (pure, tested); effects in `boot::start::initial_screen`, which runs `boot::start::boot_account` for **every** plan. The window/device/event-loop side is `wyven_app::run`, reached through the `Game` impl in `presentation::screens::shared` |
...
| Where runtime files go  | `src/infrastructure/paths.rs` — one memoised data root (`WYVEN_DATA_DIR`, else OS app-data, else CWD) with `saves_root`/`screenshots_root`/`profile_path`/`ops_path`/`keys_path`/`servers_path` off it. Engine crates never learn it: `wyven_auth::KeyCache::at` takes the path the game resolves |
...
- The module is named `core` — always reference it as `crate::core`; never write a
  bare `domain::core::` path (it would resolve to the std `core` crate).
...
- **Visual data must stay off `Block`.** `Block`'s `Debug` repr feeds
  `domain::content::Registries::hash`, which gates multiplayer joins — so anything derived
...
- **Audio never feeds `content_hash`.** `GameContent::sounds` (`assets/audio.toml`)
  ...
  into `domain::content::Registries::hash`.
...
- **A host streams chunks around every player**, not only its own
  (`presentation::screens::ingame_state::streaming`: the local render distance plus
  `SIM_RADIUS` around each remote player). Without it a client at a far altar
  would have its edits dropped (`World::set_block` returns `None` in an
  unloaded chunk) and the boss it summoned would freeze (`update_mobs` skips
  mobs in unloaded chunks). The extra chunks are simulation only; the host
  meshes what its own camera can see.
...
- **Content is identified by a string `id`, never by a number or a label.**
  ...
  digits and `_` only, enforced by `domain::core::ident::is_valid_id`; anything else
  **rejects the whole file** and falls back to the embedded builtin, because
  ...
  `[item.placeable]`, `drops`, recipe keys, worldgen and `/give` all spell. What the
  player *reads* is a separate `display_name`, defaulting to
  `domain::core::ident::title_case(id)` — and it lives on `GameContent`
...
- **Saves are id-based, not index-based.** `saves/<slug>/` stores blocks/items by
  ...
  `src/infrastructure/paths.rs`), *not* next to `assets/` — an update replaces the install
  ...
  terrain (edits still replay at their coordinates). `progression.dat` (shrines
  read, structures revealed, bosses beaten) fails soft like `mobs.dat`; a boss
  is never saved, since reloading one would replay a fight without its
  offering. `SAVE_VERSION` is **2** since the biome rings — a version-1 world
  is refused rather than loaded onto terrain it was not built on. Dropped items and arrows
  are never saved.
...
2. `cargo test --workspace` green (1113 tests, 810 of them in the game crate — `tests/architecture.rs` among them, which is what fails when an import points the wrong way). One of the game tests
