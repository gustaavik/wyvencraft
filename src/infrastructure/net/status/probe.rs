...
use crate::infrastructure::net::{
    Channel, Client, ClientMessage, PROTOCOL_ID, ServerMessage, address,
};
...
impl NetStatusProbe {
    ...
    pub fn with_client(account: &AccountState, auth: Arc<dyn AuthClient>) -> Self {
        let identity = account
            .netcode_id()
            .unwrap_or_else(crate::infrastructure::profile::local_identity);
        Self {
    ...
    fn begin(&mut self, targets: Vec<String>) {
        ...
        std::thread::spawn(move || {
            let resolved = targets
                ...
                .collect();
            let ticket = crate::infrastructure::net::ticket::issue(&account, auth.as_ref(), now)
                .map_err(|err| {
