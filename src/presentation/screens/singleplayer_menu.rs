        ...
        egui::CentralPanel::default().show(egui_ctx, |ui| {
            ui.vertical_centered(|ui| {
                ...
                if self.worlds.is_empty() {
                    ...
                    let clicks = self.draw_world_list(ui, can_host);
                    if let Some(next) = self.apply_world_clicks(clicks, &content) {
                        transition = next;
                    }
                }
                if let Some(next) = self.draw_create_form(ui) {
                    transition = next;
                }
...
/// What was clicked in the world list this frame, by world slug.
#[derive(Default)]
struct WorldClicks {
    play: Option<String>,
    host: Option<String>,
...
impl SingleplayerMenuState {
    /// The saved worlds, one row each, with Play, Host and a two-click Delete.
    fn draw_world_list(&mut self, ui: &mut egui::Ui, can_host: bool) -> WorldClicks {
        let mut clicks = WorldClicks::default();
                ...
                for world in &self.worlds {
                    ui.horizontal(|ui| {
                        ...
                        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                            let confirming =
                                self.confirm_delete.as_deref() == Some(world.slug.as_str());
                            let delete_label = if confirming { "Confirm?" } else { "Delete" };
                            if ui
                                .add_sized([70.0, 28.0], egui::Button::new(delete_label))
                                .clicked()
                            {
                                if confirming {
                                    clicks.delete = Some(world.slug.clone());
                                } else {
                            ...
                            if ui
                                ...
                                .clicked()
                            {
                                clicks.play = Some(world.slug.clone());
                            }
                            ...
                            ui.add_enabled_ui(can_host, |ui| {
                                let response =
                                    ui.add_sized([70.0, 28.0], egui::Button::new("Host"));
                                if response.clicked() {
                                    clicks.host = Some(world.slug.clone());
                                }
                                if !can_host {
                                    response
                                        .on_hover_text("Sign in to host — nobody could be let in.");
                                }
                            });
                        });
                    });
    ...
    /// Act on the world list's clicks: delete, then play, then host.
    fn apply_world_clicks(
        &mut self,
        clicks: WorldClicks,
        content: &std::sync::Arc<crate::presentation::content::GameContent>,
    ) -> Option<Transition> {
        let mut transition = None;
        if let Some(slug) = clicks.delete {
        ...
        if let Some(slug) = clicks.play {
            self.confirm_delete = None;
            transition = Some(self.play(&slug));
        }
        if let Some(slug) = clicks.host {
            self.confirm_delete = None;
            transition = Some(self.host(&slug, content.clone()));
        }
        transition
    }

    /// The new-world form: name, seed, mode, and the buttons under it.
    fn draw_create_form(&mut self, ui: &mut egui::Ui) -> Option<Transition> {
        let mut transition = None;
        // --- Create a new world ---
        ui.add_space(18.0);
        ...
        ui.add_sized(
            [260.0, 24.0],
            egui::TextEdit::singleline(&mut self.seed_text).hint_text("Seed (blank = random)"),
        );
        ui.add_space(8.0);
        ui.horizontal(|ui| {
        ...
        if ui
            ...
            .clicked()
        {
            transition = Some(Transition::Replace(Box::new(MainMenuState::new())));
        }
        transition
    }
}
