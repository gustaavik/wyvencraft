...
//! [`crate::presentation::ui::inventory`] returns an `InvAction`. It opens no files, owns no
...
use crate::presentation::editor::{
    EditorAction, EditorSession, Placement, PlacementKind, SpecPlacement,
};
...
mod tests {
    ...
    use crate::presentation::editor::store::InMemoryStore;
    use crate::presentation::editor::{CONTEXTS, EditorTarget};

    struct NoStamps;
    impl crate::presentation::editor::Stamps for NoStamps {
    ...
    fn session(declared: Vec<DisplayContext>) -> EditorSession {
        ...
        session.set_targets(vec![EditorTarget {
            key: crate::presentation::editor::PlacementKey::Item(ItemId(3)),
