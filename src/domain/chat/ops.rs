...
//!
//! Keyed by **account id** rather than by [`PlayerId`](crate::infrastructure::net::PlayerId):
...
//!
//! Reading the file is [`crate::infrastructure::ops::load_ops`] — fail-soft: an
//! absent or malformed `ops.toml` ops nobody rather than taking the game down.
//! Only the authority ever loads it — a client has nothing to decide. What is
//! here is the rule: parsing the list, and asking it who is an op.
...
/// The file an ops list is read from, in the data directory. Named here so
/// every message about it says the same thing; *where* that directory is
/// belongs to [`crate::infrastructure::paths`].
